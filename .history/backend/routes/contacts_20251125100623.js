...
// -----------------------------
// Helper: Universal Header Reader
// -----------------------------
...
// -----------------------------
// 📤 EXCEL UPLOAD — UNIVERSAL HEADER SUPPORT
// -----------------------------
router.post(
  "/upload",
  authenticate,
  ...
  async (req, res) => {
    try {
      ...
      if (!data.length) {
        return res.json({ message: "No rows found in file", count: 0 });
      }

      const { assigned_to } = req.body;
      let count = 0;

      // Map Excel rows → DB rows
      const rowsToInsert = data
        .map((row) => {
          const name = getValue(row, [
            "Name", "Full Name", "Customer Name", "Contact Name", "Candidate Name",
            "Client Name", "Employee Name", "Person", "Customer", "name"
          ]);

          const phone = getValue(row, [
            "Phone", "Phone No", "Phone Number", "Mobile", "Mobile No",
            "Mobile Number", "Contact", "Contact Number", "Whatsapp",
            "Caller Number", "Number", "phone"
          ]);
